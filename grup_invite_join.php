...
if (empty($inviteCode)) {
    header("Location: index.php?msg=invite_code_required");
    exit;
}
...
if (mysqli_num_rows($inviteResult) === 0) {
    header("Location: index.php?msg=invite_invalid");
    exit;
}
...
if ($invite['ExpiresAt'] && strtotime($invite['ExpiresAt']) < time()) {
    header("Location: index.php?msg=invite_expired");
    exit;
}

// Check max uses
if ($invite['MaxUses'] && $invite['UsedCount'] >= $invite['MaxUses']) {
    header("Location: index.php?msg=invite_limit_reached");
    exit;
}
...
if (mysqli_stmt_get_result($stmt2)->num_rows > 0) {
    header("Location: index.php?tab=grup&group=$kodeGrup&msg=already_member");
    exit;
}
...
if (mysqli_stmt_execute($stmt3)) {
    ...
    header("Location: index.php?tab=grup&group=$kodeGrup&msg=joined_group");
} else {
    header("Location: index.php?msg=join_failed");
}
?>
