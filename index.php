...
// 3. LOGIKA DEADLINE OVERVIEW (LIMIT 2)
...
$queryListTugas = "SELECT t.*, m.NamaMK FROM tugas t 
                   JOIN matakuliah m ON t.KodeMK = m.KodeMK 
                   WHERE t.StatusTugas = 'Aktif' AND t.NIM = '$nim'
...
<body>
            ...
            <div class="tab-pane fade show active" id="pills-overview">
                <div class="row">
                            ...
                            <div class="card-body">
                                ...
                                <div class="p-3 bg-light rounded mt-3">
                                    ...
                                    <div class="calendar-grid">
                                        <?php for($i=1; $i < $hariPertama; $i++) echo '<div class="calendar-date empty"></div>';
                                        for($d=1; $d <= $jumlahHari; $d++) { $cls = ($d == date('d')) ? 'today' : ''; echo "<div class='calendar-date $cls'>$d</div>"; } ?>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="col-md-6 mb-4">
                         <div class="card border-0 shadow-sm h-100">
                            <div class="card-body">
                                <h5 class="card-title fw-bold mb-3">Deadline Terdekat</h5>
                                <?php while($t = mysqli_fetch_assoc($resDeadlineOverview)): ?>
                                <div class="border rounded p-3 mb-2">
                                    <div class="d-flex justify-content-between align-items-center">
                                        <div>
                                            <h6 class="mb-1 fw-bold"><?= htmlspecialchars($t['JudulTugas']) ?></h6>
                                            <small class="text-muted"><?= $t['KodeMK'] ?></small>
                                        </div>
                                        <span class="badge bg-danger">Mendesak</span>
                                    </div>
                                    <div class="mt-2 small text-muted">
                                        <i class="fa-regular fa-clock me-1"></i> <?= date('d M Y', strtotime($t['Deadline'])) ?>
                                    </div>
                                </div>
                                <?php endwhile; ?>
                            </div>
    ...
    <div class="modal fade" id="modalTugas" tabindex="-1">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content border-0">
                <div class="modal-header border-0 pb-0"><div><h5 class="modal-title fw-bold">Tambah Tugas Baru</h5></div><button type="button" class="btn-close mb-auto" data-bs-dismiss="modal"></button></div>
                <div class="modal-body pt-3">
                    <form action="proses_tambah.php" method="POST">
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-6"><label class="form-label fw-bold small mb-1">Prioritas</label><select name="prioritas" class="form-select"><option>Sedang</option><option>Tinggi</option></select></div>
                        </div>
                        <div class="mb-4"><label class="form-label fw-bold small mb-1">Deskripsi</label><textarea name="deskripsi" class="form-control" rows="3"></textarea></div>
    ...
    <div class="modal fade" id="modalJadwal" tabindex="-1">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content border-0">
                <div class="modal-header border-0 pb-0"><div><h5 class="modal-title fw-bold">Tambah Jadwal Kuliah</h5></div><button type="button" class="btn-close mb-auto" data-bs-dismiss="modal"></button></div>
                <div class="modal-body pt-3">
                    <form action="proses_tambah.php" method="POST">
