...
if (mysqli_num_rows($result) === 0) {
    header("Location: index.php?tab=grup&msg=group_not_found");
    exit;
}
...
if (empty($group['Role'])) {
    header("Location: index.php?tab=grup&msg=not_member");
    exit;
}
    ...
    <div class="max-w-4xl mx-auto min-h-screen shadow-xl bg-white">
        ...
        <div class="bg-gradient-to-r from-purple-600 to-indigo-600 text-white p-4">
            <div class="flex items-center gap-3 mb-3">
                <button onclick="window.location.href='index.php?tab=grup'" class="w-10 h-10 rounded-full bg-white/20 hover:bg-white/30 flex items-center justify-center transition-all">
    ...
    <div id="modalInvite" class="modal hidden fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-5">
        <div class="bg-white rounded-2xl p-6 max-w-md w-full">
            ...
            <div id="inviteContent">
                <button onclick="generateInvite()" class="w-full py-3 px-4 bg-gradient-to-r from-purple-600 to-indigo-600 text-white rounded-lg font-medium hover:shadow-lg">
                    <i class="fas fa-link mr-2"></i>Generate Kode Undangan
                </button>
            </div>
        </div>
    </div>
